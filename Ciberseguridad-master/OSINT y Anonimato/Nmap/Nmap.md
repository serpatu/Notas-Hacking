...
``
```
sudo nmap -p- -sS -sC -sV --min-rate 5000 -n -Pn <IP_DE_METASPLOITABLE>
```
...
*  **-sV** (Version Detection): Detección de Versiones. En lugar de decirte solo que el puerto 80 está abierto, interroga al servicio para saber exactamente qué software corre (ej: *Apache 2.4.41*). Es fundamental para buscar vulnerabilidades específicas (CVEs) de esa versión. 

* **-sC** (Default Scripts): Scripts por Defecto. Activa el motor de scripts de Nmap (NSE) y lanza un conjunto de scripts básicos y seguros. Sirve para encontrar cosas obvias rápidamente, como login anónimo en FTP, títulos de webs o claves SSH públicas.
